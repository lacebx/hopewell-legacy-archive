...
function withIframeEmbedHeaders(response) {
...
async function getServerEntry() {
  if (!serverEntryPromise) {
    serverEntryPromise = import("./server-CCCXzOCj.mjs").then((n) => n.s).then(
      (m) => m.default ?? m
    );
  }
...
const server = {
  async fetch(request, env, ctx) {
    try {
      ...
      return withIframeEmbedHeaders(normalized);
    } catch (error) {
      console.error(error);
      return withIframeEmbedHeaders(brandedErrorResponse());
    }
  }
};
export {
